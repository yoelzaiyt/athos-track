...
import { TpmsModal } from '../components/common/TpmsModal';
import { RemoteCommandsModal } from '../components/common/RemoteCommandsModal';
import { AssetPairingFormModal } from '../components/common/AssetPairingFormModal';
...
export const FleetModule: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'veiculos' | 'motoristas' | 'rotas' | 'manutencao' | 'pareamento'>(
    'veiculos'
  );
  const [showMap, setShowMap] = useState(false);
  const [cameraAsset, setCameraAsset] = useState<AssetDevice | null>(null);
  const [tpmsAsset, setTpmsAsset] = useState<AssetDevice | null>(null);
  const [commandsAsset, setCommandsAsset] = useState<AssetDevice | null>(null);
  const [isPairingModalOpen, setIsPairingModalOpen] = useState(false);
  const [routeTemplateDraft, setRouteTemplateDraft] = useState({ name: '', origin: '', destination: '', estDistanceKm: '', estDurationMin: '' });
  ...
  const driverColumns: Column<Driver>[] = [
    ...
    {
      header: 'Pontuação de Condução (CAN)',
      accessor: (row) => {
        if (row.drivingScore === undefined) return <span className="text-slate-400 dark:text-slate-600">—</span>;
        const scoreColor = row.drivingScore >= 80 ? 'text-emerald-600 dark:text-emerald-400' : row.drivingScore >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400';
        return (
          <div className="flex flex-col gap-1 min-w-[150px]">
            <div className="flex items-center justify-between">
              <span className={`flex items-center gap-1 font-bold font-mono ${scoreColor}`}>
                <Activity className="w-3.5 h-3.5" /> {row.drivingScore}/100
              </span>
            </div>
            {row.harshEventsToday && (
              <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">
                {row.harshEventsToday.suddenAcceleration} acel. · {row.harshEventsToday.suddenBraking} freadas ·{' '}
                {row.harshEventsToday.sharpTurn} curvas · {row.harshEventsToday.excessiveIdlingMinutes}min parado
              </span>
            )}
          </div>
        );
  ...
  const tripColumns: Column<TripRecord>[] = [
    ...
    {
      header: 'Modelo de Rota (Yaw)',
      accessor: (row) => {
        if (!row.routeTemplateId) return <span className="text-slate-400 dark:text-slate-600">Rota livre</span>;
        const tpl = routeTemplates.find((t) => t.id === row.routeTemplateId);
        return row.deviationDetected ? (
          <span className="px-2 py-0.5 text-[10px] font-bold font-mono rounded-full uppercase bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 flex items-center gap-1 w-fit">
            <AlertTriangle className="w-3 h-3" /> Desvio detectado
          </span>
        ) : (
          <span className="text-slate-500 dark:text-slate-400 text-[11px] truncate max-w-[160px] block">{tpl?.name || 'Modelo vinculado'}</span>
        );
      },
    },
  ];
    ...
    <div className="p-6 space-y-6 bg-slate-50 dark:bg-slate-950 min-h-screen text-slate-900 dark:text-slate-100 transition-colors">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-1 rounded-xl text-xs shadow-sm">
          ...
          <button
            onClick={() => setActiveSubTab('pareamento')}
            className={`px-3 py-1.5 rounded-lg font-medium transition-colors ${
              activeSubTab === 'pareamento' ? 'bg-amber-500 text-slate-950 font-bold' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
            }`}
          >
            Pareamento ({assetPairings.length})
          </button>
        </div>
      </div>
          ...
          <DataTable
            ...
            actions={(item) => (
              <div className="flex items-center justify-end gap-1.5">
                ...
                {!!item.tirePositions?.length && (
                  <button
                    onClick={() => setTpmsAsset(item)}
                    className={`p-1.5 rounded-lg transition-colors ${
                      item.tirePositions.some((t) => t.status !== 'normal')
                        ? 'bg-rose-500/10 text-rose-600 dark:text-rose-400 hover:bg-rose-500/20'
                        : 'bg-slate-100 dark:bg-slate-800 hover:bg-emerald-500/15 text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400'
                    }`}
                    title="Ver TPMS (pressão dos pneus)"
                  >
                    <Gauge className="w-3.5 h-3.5" />
                  </button>
                )}
                <button
                  onClick={() => setCommandsAsset(item)}
                  className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-indigo-500/15 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                  title="Comandos remotos operacionais"
                >
      ...
      {activeSubTab === 'rotas' && (
        <>
          ...
          {/* Modelos de Rota Reutilizáveis (Yaw/YawTrip) */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm space-y-3">
            <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
              <Navigation2 className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
              Modelos de Rota Reutilizáveis (detecção de desvio)
            </h3>
            <div className="flex flex-wrap gap-2">
              {routeTemplates.map((t) => (
                <span
                  key={t.id}
                  className="flex items-center gap-2 px-2.5 py-1.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-[11px] text-slate-600 dark:text-slate-300"
                >
                  {t.name} <span className="text-slate-400 dark:text-slate-500 font-mono">({t.estDistanceKm}km)</span>
                  <button onClick={() => deleteRouteTemplate(t.id)} className="text-slate-400 hover:text-rose-500 transition-colors">
                    <Trash2 className="w-3 h-3" />
                  </button>
                </span>
              ))}
              {routeTemplates.length === 0 && <span className="text-[11px] text-slate-400 dark:text-slate-600">Nenhum modelo cadastrado.</span>}
            </div>
            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
              <input
                type="text"
                value={routeTemplateDraft.name}
                onChange={(e) => setRouteTemplateDraft({ ...routeTemplateDraft, name: e.target.value })}
                placeholder="Nome do modelo"
                className="flex-1 min-w-[140px] bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1.5 text-xs text-slate-900 dark:text-slate-200 focus:outline-none"
              />
              <input
                type="text"
                value={routeTemplateDraft.origin}
                onChange={(e) => setRouteTemplateDraft({ ...routeTemplateDraft, origin: e.target.value })}
                placeholder="Origem"
                className="flex-1 min-w-[120px] bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1.5 text-xs text-slate-900 dark:text-slate-200 focus:outline-none"
              />
              <input
                type="text"
                value={routeTemplateDraft.destination}
                onChange={(e) => setRouteTemplateDraft({ ...routeTemplateDraft, destination: e.target.value })}
                placeholder="Destino"
                className="flex-1 min-w-[120px] bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1.5 text-xs text-slate-900 dark:text-slate-200 focus:outline-none"
              />
              <input
                type="number"
                value={routeTemplateDraft.estDistanceKm}
                onChange={(e) => setRouteTemplateDraft({ ...routeTemplateDraft, estDistanceKm: e.target.value })}
                placeholder="km"
                className="w-20 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1.5 text-xs font-mono text-slate-900 dark:text-slate-200 focus:outline-none"
              />
              <input
                type="number"
                value={routeTemplateDraft.estDurationMin}
                onChange={(e) => setRouteTemplateDraft({ ...routeTemplateDraft, estDurationMin: e.target.value })}
                placeholder="min"
                className="w-20 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1.5 text-xs font-mono text-slate-900 dark:text-slate-200 focus:outline-none"
              />
              <button
                onClick={() => {
                  if (!routeTemplateDraft.name.trim()) return;
                  addRouteTemplate({
                    id: `rtpl_${Date.now()}`,
                    name: routeTemplateDraft.name.trim(),
                    clientId: selectedClientId === 'all' ? 'cli_1' : selectedClientId,
                    unitId: selectedUnitId === 'all' ? 'unit_1' : selectedUnitId,
                    origin: routeTemplateDraft.origin,
                    destination: routeTemplateDraft.destination,
                    waypoints: [],
                    estDistanceKm: Number(routeTemplateDraft.estDistanceKm) || 0,
                    estDurationMin: Number(routeTemplateDraft.estDurationMin) || 0,
                  });
                  setRouteTemplateDraft({ name: '', origin: '', destination: '', estDistanceKm: '', estDurationMin: '' });
                }}
                className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-semibold text-xs flex items-center gap-1 transition-colors"
              >
                <Plus className="w-3.5 h-3.5" /> Salvar Modelo
              </button>
            </div>
          </div>
      ...
      {/* ===================== PAREAMENTO & PROXIMIDADE ===================== */}
      {activeSubTab === 'pareamento' && (
        <>
          <div className="p-3.5 bg-indigo-500/5 border border-indigo-500/20 rounded-xl text-[11px] text-slate-500 dark:text-slate-400">
            Alerta de proximidade entre dois ativos pareados (ex.: caminhão + carga em escolta, veículo + operador). Se a
            distância entre os dois ultrapassar o limite configurado, o par é sinalizado como separado.
          </div>
          ...
          <div className="flex justify-end">
            ...
            >
              <Plus className="w-4 h-4" />
              <span>Novo Pareamento</span>
            </button>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
            {assetPairings.length === 0 ? (
              <div className="col-span-full py-8 text-center text-slate-400 dark:text-slate-600">
                Nenhum pareamento configurado.
              </div>
            ) : (
              assetPairings.map((pair) => {
                const primary = getFilteredAssets('all', 'all').find((a) => a.id === pair.primaryAssetId);
                const secondary = getFilteredAssets('all', 'all').find((a) => a.id === pair.secondaryAssetId);
                const distance =
                  primary && secondary
                    ? Math.round(
                        haversineMeters(
                          primary.telemetry.latitude,
                          primary.telemetry.longitude,
                          secondary.telemetry.latitude,
                          secondary.telemetry.longitude
                        )
                      )
                    : null;
                const separated = distance !== null && distance > pair.maxDistanceMeters;
                return (
                  <div
                    key={pair.id}
                    className={`p-3.5 rounded-xl border space-y-2 ${
                      separated ? 'bg-rose-500/5 border-rose-500/30' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'
                    }`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button
                        onClick={() => {
                          if (window.confirm(`Remover o pareamento "${pair.label}"?`)) deleteAssetPairing(pair.id);
                        }}
                    ...
                    <div className="text-slate-500 dark:text-slate-400 text-[11px] space-y-0.5">
                      <div>{pair.primaryAssetName}</div>
                      <div>{pair.secondaryAssetName}</div>
                    </div>
                    <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-800 text-[11px]">
                      <span className="text-slate-500 dark:text-slate-400">Limite: {pair.maxDistanceMeters}m</span>
                      {distance !== null ? (
                        <span
                          className={`font-mono font-bold flex items-center gap-1 ${
                            separated ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'
                          }`}
                        >
                          {separated && <Ban className="w-3 h-3" />} {distance}m
                        </span>
                      ) : (
      ...
      <TpmsModal isOpen={!!tpmsAsset} onClose={() => setTpmsAsset(null)} asset={tpmsAsset} />
      <RemoteCommandsModal isOpen={!!commandsAsset} onClose={() => setCommandsAsset(null)} asset={commandsAsset} />
      <AssetPairingFormModal
        isOpen={isPairingModalOpen}
        onClose={() => setIsPairingModalOpen(false)}
        onSave={(pairing) => {
          addAssetPairing(pairing);
          setIsPairingModalOpen(false);
        }}
        assets={getFilteredAssets(selectedClientId, selectedUnitId)}
      />
    </div>
  );
};
